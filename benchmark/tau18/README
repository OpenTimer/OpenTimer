...
2. The mode is **late mode (MAX)**.
3. For leon3mp_iccad, the slacks are **positive and lower than 40000**. So you need to set up the slack upper bound
   when doing PBA. 
4. We compare the outputs with the winner in tau 2018 contest: iTimer. **The golden files are compressed as zip files (*.output)**. 
...
2. Run the tau18 binary 
3. Run the correlation.py: python3 correlation.py OT_output OpenSTA_output vga_lcd. You will see 
   three numbers: [path count] [Pearson Correlation Coefficient] [Coefficient of determination (R2)]
