...
void SDC::read(const std::filesystem::path &path) {
  ...
  OT_LOGE_RIF(sdc_home.empty() || !std::filesystem::exists(sdc_home),
  ...
  // child
    ...
    std::vector<std::string> args{OT_TCLSH_PATH, "sdc.tcl", sdc_path.c_str(),
                                  sdc_json.c_str()};
  ...
  // parent
    ...
    do {
      ...
      if (WIFEXITED(s)) {
        OT_LOGE_RIF(WEXITSTATUS(s) != EXIT_SUCCESS,
                    "sdc reader exited with failure");
...
CreateClock::CreateClock(const Json &json) {
  ...
  for (auto itr = json.begin(); itr != json.end(); ++itr) {
    if (auto &key = itr.key(); key == "-period") {
      ...
      comment = std::string(itr.value());
    } else if (key == "-name") {
      name = std::string(itr.value());
